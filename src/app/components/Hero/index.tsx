  ...
  const sliderImages = [
    {
      ...
      description: "Building sustainable solutions that empower communities worldwide",
    },
    {
      url: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=600&h=400&fit=crop&crop=center",
      title: "Education Access",
      description: "Providing educational opportunities to underserved populations",
    },
    {
      ...
      <div className="relative z-10 min-h-screen flex items-center px-6 py-20">
        <div className="max-w-7xl mx-auto w-full">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            {/* Left Side - Content */}
            <div className={`space-y-8 transition-all duration-1000 ease-out ${isVisible ? "translate-x-0 opacity-100" : "-translate-x-10 opacity-0"}`}>
              <div>
                <div className="inline-block px-4 py-2 bg-gray-50 text-gray-600 text-sm font-medium rounded-full mb-6 hover:bg-gray-100 transition-colors duration-300">Foundation • Est. 2024</div>
                <h1 className="text-4xl sm:text-5xl lg:text-6xl font-black text-gray-900 leading-none mb-6">
                  Reksa
                  <br />
                  <span className="text-transparent bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text">Mahardhika</span>
                </h1>
                <div className="w-24 h-1 bg-gradient-to-r from-blue-600 to-purple-600 mb-8 rounded-full"></div>
              </div>

              <p className="text-xl text-gray-600 leading-relaxed max-w-lg">Perkumpulan Kesejahteraan Sosial Reksa Mahardhika Utama.</p>

              <div className="flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-8">
                <button className="px-8 py-4 bg-gray-900 text-white font-semibold rounded-lg hover:bg-gray-800 hover:scale-105 transition-all duration-300 shadow-md hover:shadow-xl">Explore Our Work</button>
              ...
              <div className="flex space-x-12 pt-8 border-t border-gray-100">
                <div className="group cursor-pointer">
                  <div className="text-3xl font-bold text-gray-900 group-hover:text-blue-600 transition-colors duration-300">1,000+</div>
                  <div className="text-sm text-gray-500">Active Members</div>
                </div>
                <div className="group cursor-pointer">
                  <div className="text-3xl font-bold text-gray-900 group-hover:text-purple-600 transition-colors duration-300">50+</div>
                  <div className="text-sm text-gray-500">Active Programs</div>
                </div>
                <div className="group cursor-pointer">
                  <div className="text-3xl font-bold text-gray-900 group-hover:text-indigo-600 transition-colors duration-300">2024</div>
            ...
            <div className={`relative transition-all duration-1000 ease-out delay-300 ${isVisible ? "translate-x-0 opacity-100" : "translate-x-10 opacity-0"}`}>
                ...
                <div className="w-full h-96 rounded-2xl overflow-hidden shadow-2xl hover:shadow-3xl transition-shadow duration-500 group relative">
                  ...
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
                    {sliderImages.map((_, index) => (
                      <button key={index} onClick={() => setCurrentSlide(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentSlide ? "bg-white w-6" : "bg-white/50"}`} />
                    ))}
                  </div>
                </div>
